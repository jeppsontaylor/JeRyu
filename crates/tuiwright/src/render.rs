    ...
    pub fn render_screen(&self, screen: &ScreenSnapshot) -> RgbaImage {
        ...
        for row in 0..screen.rows {
            for col in 0..screen.cols {
                ...
                // Draw cell text using the embedded TTF and maintained rasterizer.
...
fn font() -> &'static FontArc {
    FONT.get_or_init(|| FontArc::try_from_slice(FONT_DATA).expect("Failed to load font"))
}

/// Draw a character using a maintained rasterizer, anti-aliased and scaled.
fn draw_char_ttf(
    img: &mut RgbaImage,
    x: u32,
    y: u32,
    ...
    let font = font();
    let uniform_scale = PxScale::from(cell_h as f32);
    let scaled = font.as_scaled(uniform_scale);
    let m_advance = scaled.h_advance(scaled.glyph_id('M'));
    let scale = PxScale {
        x: cell_h as f32 * (cell_w as f32 / m_advance.max(1.0)),
        y: cell_h as f32,
    };
    let scaled = font.as_scaled(scale);
    let glyph_id = scaled.glyph_id(ch);
    let glyph =
        glyph_id.with_scale_and_position(scale, point(x as f32, y as f32 + scaled.ascent()));
