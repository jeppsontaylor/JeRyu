...
This appendix is the agent-facing contract for the V3.01 repository. It is grounded in the active \apicode{vgit} binary, the Unix-socket capability API, the native MCP server, the engine HTTP routes, the GitLab Runner custom executor protocol, the Git server admission hook, and the canonical action registry. The MCP surface is not a parallel authority system: it is a standards-compatible transport for the same policy, evidence, grant, and merge/release checks exposed through the existing capability model.
...
vgit explain-blocker job 14445
vgit capability serve /tmp/vgit-capability.sock
vgit mcp serve
vgit mcp serve-http
vgit mcp tools --json
\end{lstlisting}
...
\midrule
\apicmd{vgit init} & mutating & Bootstrap GitLab, state DB, runners, secrets, compose files, and smoke setup.\\
\apicmd{vgit bootstrap} & mutating & Hidden alias for \apicode{init}.\\
\apicmd{vgit serve} & daemon & Start Docker/GitLab orchestration, hook install, cache supervisor, runner warmup, engine, and shadow loop.\\
\apicmd{vgit down} & mutating & Drain runner managers and stop the GitLab stack.\\
\apicmd{vgit status} & read & Show GitLab, Vault, pools, managed containers, jobs, release, and secret state.\\
\apicmd{vgit tui [--once]} & UI/read & Launch the Ratatui dashboard or render one smoke frame.\\
\apicmd{vgit tui --capture --tab <tab> --output <png>} & read/artifact & Deterministic Ratatui PNG capture. Tabs: mission, release, jobs, agents, tests, pools, cache, evidence, secrets.\\
\apicmd{vgit tui --screenshot --tab <tab> --screenshot-hold-ms <ms>} & read/session & Hold a real terminal render for external screenshot tooling.\\
\apicmd{vgit progress --project-id 2 --ref-name main [--json]} & read & Show lane-aware CI and release progress.\\
\apicmd{vgit next --project-id 2 --ref-name main} & read & Print the highest-priority next action for a branch.\\
\apicmd{vgit explain-blocker <job|release|merge> <id>} & read & Explain why an entity is blocked using DB/GitLab evidence.\\
\apicmd{vgit action list [--json]} & read & Emit the canonical action registry for humans or agents.\\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Command} & \textbf{Mode} & \textbf{Purpose}\\
\midrule
\apicmd{vgit pool list} & read & List pools, manager counts, paused state, executor, and runner id.\\
\apicmd{vgit pool scale <name> <count>} & mutating & Scale a pool to an exact manager count.\\
\apicmd{vgit pool pause|resume <name>} & mutating & Pause or resume runner intake.\\
\apicmd{vgit pool drain <name>} & mutating & Pause, wait, and stop managers.\\
\apicmd{vgit pool delete <name>} & mutating & Drain, delete DB record, and unregister runner.\\
\apicmd{vgit pool rotate-token <name>} & mutating & Reset GitLab runner token and update manager config.\\
\apicmd{vgit job list <project> [--status running,pending]} & read & List GitLab jobs by scope.\\
\apicmd{vgit job trace <project> <job>} & read & Print GitLab job trace.\\
\apicmd{vgit job play|cancel|retry <project> <job>} & mutating & Start manual jobs, cancel jobs, or retry failed jobs.\\
\apicmd{vgit job explain <project> <job>} & read & Print latest structured failure capsule and retry advice.\\
\apicmd{vgit job clear} & mutating & Clear local job and pipeline history.\\
\apicmd{vgit pipeline explain|doctor|jobs <pipeline> [--project-id 2] [--json]} & read & Inspect release eligibility, health, and job timing.\\
\apicmd{vgit pipeline jobs <pipeline> --ingest} & mutating & Store current job timing rows.\\
\apicmd{vgit pipeline ingest|cancel <pipeline> [--project-id 2]} & mutating & Persist timings or cancel a pipeline.\\
\apicmd{vgit pipeline bottlenecks [--limit 25] [--json]} & read & Report historical slow jobs from local timing ledger.\\
\apicmd{vgit cache enable} & mutating & Configure Docker for SmartCache registry mirror.\\
\apicmd{vgit cache doctor|status [--json]} & read & Check proxy/registry/cache state and metrics.\\
\apicmd{vgit cache gc [--dry-run] [--json] [--older-than <age>] [--max-cache-gb <gb>]} & mutating & Garbage-collect manager caches and cache state.\\
\apicmd{vgit logs <manager-id> [-n 50]} & read & Tail manager container logs.\\
\apicmd{vgit host storage-audit|doctor [--json]} & read & Audit storage and host health.\\
\apicmd{vgit host reclaim --mode <mode> (--plan|--apply)} & mutating & Run planned or applied reclaim.\\
\bottomrule
\end{tabular}
\end{center}

\clearpage
\apitabletitle{Agent, Shadow, Test/VTI, Release, Secret, and Repo Commands}
\begin{center}
\small
...
\textbf{Command} & \textbf{Mode} & \textbf{Purpose}\\
\midrule
\apicmd{vgit agent spawn <project> --task <text>} & mutating & Create an autonomous task branch/MR/issue workflow.\\
\apicmd{vgit agent list <project>} & read & List active agent MRs/issues.\\
\apicmd{vgit agent merge <project> <mr> [--trust-tier trusted]} & mutating & Request merge through the risk gate.\\
\apicmd{vgit shadow <subcommand>} & mixed & Manage periodic local source-to-GitLab shadow sync: add, enable, disable, remove, sync-now, status.\\
\apicmd{vgit shadow-remote <subcommand>} & mixed & Inspect, create, or push a repo-local shadow remote: status, ensure, push.\\
\apicmd{vgit test run --command <cmd> [--force]} & mutating & Run a command through an ephemeral GitLab CI pipeline.\\
\apicmd{vgit test plan --command <cmd>} & read & Preview inferred image, tags, timeout, and runner class.\\
\apicmd{vgit test batch --command <cmd>... [--max-parallel 3]} & mutating & Launch several validation pipelines concurrently.\\
\apicmd{vgit test results|failed|retry <pipeline>} & mixed & Inspect or retry pipeline test jobs.\\
\apicmd{vgit test impact --base <sha> --head <sha> [--json]} & read & Determine CI jobs and release gates needed for a diff.\\
\apicmd{vgit test select [emit flags]} & read/artifact & Compute internal VTI plan and optionally emit GitLab YAML, JSON plan, and VTI receipt.\\
\apicmd{vgit test explain-plan <plan.json>} & read & Explain a serialized VTI plan.\\
\apicmd{vgit test select-external --workspace <root>} & read/artifact & Use external \apicode{.vgit/testmap.toml}.\\
\apicmd{vgit test audit|learn} & read & Compare VTI selections with oracle results and suggest rule changes.\\
\apicmd{vgit test cache-status --base <sha> --head <sha> [--json]} & read & Show test-command cache state for the current source state.\\
\apicmd{vgit release status|watch|reconcile} & mixed & Read or reconcile release attempts.\\
\apicmd{vgit release promote-prod [--version <v>]} & mutating & Trigger approved production promotion after canary gates.\\
\apicmd{vgit release preflight|doctor [--json]} & read & Check blockers, infrastructure, and release readiness.\\
\apicmd{vgit secrets <subcommand>} & mixed & Manage Vault-backed release lifecycle: init, status, rotate, finalize, report, recover.\\
\apicmd{vgit repo render-agent-index [--check]} & artifact/check & Generate or verify machine-readable agent routing index.\\
\apicmd{vgit repo audit-agent-surface [--json]} & read & Audit routing docs, generated index, and agent surface freshness.\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{lstlisting}[style=api,language=bash,caption={Exhaustive vgit command forms}]
vgit init
vgit bootstrap
vgit serve
vgit down
vgit status
vgit logs <manager-id> [-n <lines>]
vgit progress [--project-id <id>] [--ref-name <ref>] [--json]
vgit next [--project-id <id>] [--ref-name <ref>]
vgit explain-blocker <job|release|merge> <id>

vgit tui [--once]
vgit tui --capture [--tab <tab>] [--output <path>] [--width <cols>] [--height <rows>]
vgit tui --screenshot [--tab <tab>] [--width <cols>] [--height <rows>] [--screenshot-hold-ms <ms>]
...
vgit job list <project-id> [--status <csv>]
vgit job trace <project-id> <job-id>
vgit job play <project-id> <job-id>
vgit job cancel <project-id> <job-id>
vgit job retry <project-id> <job-id>
vgit job explain <project-id> <job-id>
vgit job clear

vgit pipeline explain [--project-id <id>] <pipeline-id> [--json]
vgit pipeline doctor [--project-id <id>] <pipeline-id> [--json]
vgit pipeline jobs [--project-id <id>] <pipeline-id> [--ingest] [--json]
vgit pipeline ingest [--project-id <id>] <pipeline-id>
vgit pipeline cancel [--project-id <id>] <pipeline-id>
vgit pipeline bottlenecks [--project-id <id>] [--ref-name <ref>] [--limit <n>] [--json]
...
vgit agent spawn <project-id> --task <text>
vgit agent list <project-id>
vgit agent merge <project-id> <mr-iid> [--trust-tier <tier>]

vgit shadow add --source <path> --project-id <id> --branch <branch> [--enable]
vgit shadow enable --source <path>
vgit shadow disable --source <path>
vgit shadow remove --source <path>
vgit shadow sync-now --source <path>
vgit shadow status [--source <path>]

vgit shadow-remote status [--repo <path>] [--name <remote>]
vgit shadow-remote ensure [--repo <path>] [--name <remote>] --url <url>
vgit shadow-remote push [--repo <path>] [--name <remote>] [--branch <branch>] [--mirror]

vgit test run --command <cmd> [--project-id <id>] [--image <image>] [--tags <csv>] [--timeout <sec>] [--force]
vgit test plan --command <cmd> [--project-id <id>] [--image <image>] [--tags <csv>] [--timeout <sec>]
vgit test batch --command <cmd>... [--project-id <id>] [--image <image>] [--tags <csv>] [--timeout <sec>] [--max-parallel <n>] [--force]
vgit test results <pipeline-id> [--project-id <id>]
vgit test retry <pipeline-id> <job-name> [--project-id <id>]
vgit test failed <pipeline-id> [--project-id <id>]
vgit test impact --base <ref> --head <ref> [--repo-root <path>] [--json]
vgit test select [--base <ref>] [--head <ref>] [--repo-root <path>] [--explain] [--json] [--emit-gitlab <path>] [--emit-plan <path>] [--emit-receipt <path>]
vgit test explain-plan <plan-path>
vgit test select-external [--base <ref>] [--head <ref>] --workspace <path> [--explain] [--json] [--emit-gitlab <path>] [--emit-plan <path>] [--emit-skipped <path>]
vgit test audit --changed <csv> --all-tests <csv> [--failed <csv>] [--sha <sha>] [--json] [--workspace <path>]
vgit test learn --changed <csv> --all-tests <csv> [--failed <csv>] [--sha <sha>] [--json] [--workspace <path>]
vgit test cache-status [--base <ref>] [--head <ref>] [--json]

vgit release status [--project-id <id>] [--ref-name <ref>] [--sha <sha>] [--limit <n>] [--json]
vgit release watch [--project-id <id>] [--ref-name <ref>] [--sha <sha>] [--limit <n>] [--interval-secs <sec>] [--json]
vgit release reconcile [--project-id <id>] [--ref-name <ref>] [--json]
vgit release promote-prod [--project-id <id>] [--ref-name <ref>] [--version <version>]
vgit release preflight [--ssh-host <host>] [--json]
vgit release doctor [--version <version>] [--preflight[=true|false]] [--json]

vgit secrets init
vgit secrets status [--json]
vgit secrets rotate [--repo <name>] --version <version> --target <target>
vgit secrets finalize [--repo <name>] --version <version> --target <target>
vgit secrets report [--repo <name>] --version <version>
vgit secrets recover [--repo <name>] --version <version>

vgit host storage-audit
vgit host doctor [--json]
vgit host reclaim --mode <mode> [--plan] [--apply]
...
vgit exec config
vgit exec prepare
vgit exec run <script-path> <stage>
vgit exec cleanup
vgit server-hook pre-receive
vgit capability serve <socket-path>
\end{lstlisting}
...
\midrule
\apicmd{vgit exec config} & GitLab Runner & Return custom executor driver configuration JSON.\\
\apicmd{vgit exec prepare} & GitLab Runner & Prepare clone/sandbox/honeypot/cache context.\\
\apicmd{vgit exec run <script> <stage>} & GitLab Runner & Run a build stage with cache brain, witnesses, tripwire handling, and capsule capture.\\
\apicmd{vgit exec cleanup} & GitLab Runner & Clean executor state after job completion.\\
\apicmd{vgit server-hook pre-receive} & Git server hook & Read ref updates from stdin and write admission decisions.\\
\apicmd{vgit capability serve <socket>} & local supervisor & Start Unix-socket JSON capability API.\\
\bottomrule
\end{tabular}
\end{center}
...
Legacy requests & Accepted for compatibility, tagged as \apicode{legacy-capability-client}.\\
\bottomrule
\end{tabular}
\end{center}
...

{
  ...
  "payload": {
    "project_id": 2,
    "ref_name": "main",
    "test_ids": ["unit:vgit", "integration:job_tests"]
  }
}
\end{lstlisting}
...
POST & \apicode{/hooks} & \apicode{X-Gitlab-Token} matching \apicode{VGIT\_WEBHOOK\_SECRET} & Ingest GitLab job, pipeline, push, and MR webhooks.\\
...
curl -X POST http://127.0.0.1:9777/hooks \
  -H "X-Gitlab-Token: $VGIT_WEBHOOK_SECRET" \
...
Push Hook & Normalize ref, skip \apicode{vgit-test-*}, cancel superseded pipelines, compute impact, persist VTI plan.\\
...
The action registry is the shared action catalog consumed by the TUI command palette, \apicode{vgit action list}, and \apicode{ListAllowedActions}. Each entry exposes \apicode{id}, \apicode{label}, \apicode{key\_hint}, \apicode{risk\_tier}, \apicode{side\_effect\_class}, \apicode{required\_grant}, \apicode{dry\_run}, \apicode{surfaces}, and \apicode{description}.
...
V3.01 ships a native Model Context Protocol server for external coding agents that already speak MCP. The server is implemented as \apicode{src/mcp.rs} plus the \apicode{src/mcp/} module tree and is exposed through \apicode{vgit mcp serve}, \apicode{vgit mcp serve-http}, and \apicode{vgit mcp tools --json}. Its purpose is deliberately narrow: translate MCP lifecycle and tool messages into the same typed \apicode{AgentIntent} execution path used by the Unix-socket capability API.

The important boundary is that MCP is a transport, not a privilege escalation path. A call to \apicode{vgit.request\_merge} over MCP is evaluated by the same capability policy, grant metadata, durable evidence recording, GitLab client wrapper, and merge/release gate logic as the equivalent native capability request. Read-only tools remain read-only; branch-writing tools still create explicit branches and evidence; production-sensitive tools still require the same gate decisions.
...
\midrule
\shortstack[l]{\apicode{vgit.get\_system}\\\apicode{\_snapshot}} & Read current operational state for jobs, pools, release, and evidence.\\
\shortstack[l]{\apicode{vgit.explain}\\\apicode{\_blockers}} & Explain why a job, release, or merge is blocked.\\
\shortstack[l]{\apicode{vgit.plan}\\\apicode{\_validation}} & Produce a VTI-backed validation plan for selected tests and refs.\\
\shortstack[l]{\apicode{vgit.fetch}\\\apicode{\_capsule}} & Fetch structured failure capsule data for a job.\\
\shortstack[l]{\apicode{vgit.run}\\\apicode{\_tests}} & Create an ephemeral validation branch and trigger CI for a scoped test run.\\
\shortstack[l]{\apicode{vgit.propose}\\\apicode{\_patch}} & Create a branch, apply file modifications, commit, and open a merge request.\\
\shortstack[l]{\apicode{vgit.race}\\\apicode{\_patches}} & Launch multiple patch hypotheses while preserving branch-level evidence.\\
\shortstack[l]{\apicode{vgit.request}\\\apicode{\_merge}} & Ask the merge gate to evaluate an MR against release and risk policy.\\
\bottomrule
\end{tabular}
\end{table}
