...
Git compatibility is handled through \apicode{jeryu git <args>} passthrough plus the native \apicode{jeryu save}, \apicode{jeryu sync}, and \apicode{jeryu undo} wrappers. The removed \apicode{ship} command and the old repo-mirror entrypoints no longer own a separate shadow-git control plane.
...
\apicmd{jeryu git <args>} & mixed & Pass through to the system Git binary and preserve the hook-driven shadow-git path.\\
