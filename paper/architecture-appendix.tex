...
\node[lane=teal, minimum width=8.88cm, minimum height=6.95cm, anchor=north west] (vgit) at (3.75,7.45) {};
\node[title, text=teal!55!black, anchor=west] at (3.95,7.18) {vgit Rust control plane};
...
\node[box=orange, anchor=north west] at (10.12,4.75) {Release\\shadow};
...
\node[extbox=orange, anchor=north west] (repos) at (13.32,2.18) {Shadow repos\\branches, refs};
...
\end{tikzpicture}
\end{adjustbox}
\caption{JeRyu architecture as described in \texttt{docs/ARCHITECTURE.md}. GitLab remains the forge and CI substrate, while the Rust \texttt{vgit} control plane owns agent authority, runner policy, validation planning, cache trust, evidence capture, merge gates, and release gates.}
