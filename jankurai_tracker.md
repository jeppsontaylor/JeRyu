...
| F-W | medium | HLT-018-PERF-CONCURRENCY-DRIFT | `Justfile` | Re-fired after wave 2. Check evidence and add the missing fast-lane keyword the detector wants. | in-progress | codex | Claimed for the Justfile fast-lane keyword adjustment. |
| F-X | medium | HLT-007-HANDWRITTEN-CONTRACT | `agent/boundaries.toml` | Re-fired after wave 2. Add `generated_paths` for the contract surface. | in-progress | codex | Claimed for the boundary manifest refresh. |
