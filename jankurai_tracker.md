...
| F-AP | high | HLT-000-SCORE-DIMENSION (dup block) | `src/release.rs:1` | Duplicate release pipeline block detected. Extract the shared behavior behind one named boundary and add focused tests before changing behavior. | in-progress | codex | Claimed from the current audit snapshot. |
