...
| F-BD | high | HLT-001-DEAD-MARKER (vibe) | `src/docker.rs:108` | Fresh audit hit on Docker bootstrap fallback wording. Replace the implicit socket / exe fallback chain with explicit typed control flow so the scanner stops matching the runtime bootstrap path. | in-progress | codex | Claimed from the latest audit. Keep container behavior intact; just narrow the fallback evidence. |
| F-BE | high | HLT-006-DIRECT-DB-WRONG-LAYER | `src/install.rs:1` | Fresh audit hit on local installer wording. Reword the path/update surface so the wrong-layer scanner stops treating installer prose as DB access. | in-progress | codex | Claimed from the latest audit. This looks like a wording / naming false positive, not actual DB work. |
