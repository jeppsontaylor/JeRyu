...
//! Proof: `cargo test -p vgit -- secrets`
...
const OPS_POLICY_NAME: &str = "vgit-release-ops";
const OPS_DISPLAY_NAME: &str = "vgit-release-control-plane";
...
fn load_vault_env() -> Result<Option<VaultEnv>> {
    ...
    let Some(addr) = parsed.get("VGIT_VAULT_ADDR").cloned() else {
        return Ok(None);
    };
    let Some(token) = parsed.get("VGIT_VAULT_TOKEN").cloned() else {
        return Ok(None);
    };
    Ok(Some(VaultEnv {
        addr,
...
fn save_vault_env(env: &VaultEnv) -> Result<()> {
    ...
    values.insert("VGIT_VAULT_ADDR".to_string(), env.addr.clone());
    values.insert("VGIT_VAULT_TOKEN".to_string(), env.token.clone());
    values.insert("VGIT_VAULT_MOUNT".to_string(), env.mount.clone());
    values.insert("VGIT_VAULT_PREFIX".to_string(), env.prefix.clone());
...
async fn build_operational_env() -> Result<VaultEnv> {
    ...
    let root_password = ensure_vgit_compose_inputs()?;
    write_restricted(
        &config::compose_file(),
...
async fn ensure_release_envs(
    ...
    deploy_env.insert(
        ...
        format!("vgit-deploy-lease-{}", random_alnum(16)),
    );
    deploy_env.insert(
    ...
    let rendered_deploy_env = repo_root
        .join("ops/releases")
        .join(version)
        .join("rendered/vgit.prod.deploy.env");
