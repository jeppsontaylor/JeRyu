...
//! Proof: `cargo nextest run -p vgit -- test_intel::testmap`
//! Invariants: Parsed test maps preserve lane semantics and reject ambiguous ownership where possible.
//! Parses `.vgit/testmap.toml` files for external workspace integration.
//!
...
/// Load and parse a `.vgit/testmap.toml` file.
...
/// Compute a test plan from a `.vgit/testmap.toml` and a list of changed paths.
...
fn emit_child_plan_context(plan: &ExternalTestPlan) -> String {
    ...
    format!(
        "plan-tests:\n\
         ...
         \x20 tags: [default]\n\
         \x20 script:\n\
         \x20   - mkdir -p target/vgit\n\
         \x20   - |\n\
         \x20     cat > target/vgit/changed-files.txt <<'VTI_CHANGED_FILES'\n\
{changed_paths}         \x20     VTI_CHANGED_FILES\n\
         \x20   - |\n\
         \x20     cat > target/vgit/vti-plan.json <<'VTI_PLAN_JSON'\n\
{plan_json}         \x20     VTI_PLAN_JSON\n\
         \x20   - |\n\
         \x20     cat > target/vgit/vti-skipped.json <<'VTI_SKIPPED_JSON'\n\
{skipped_json}         \x20     VTI_SKIPPED_JSON\n\
         \x20 artifacts:\n\
         \x20   when: always\n\
         \x20   expire_in: 7 days\n\
         \x20   paths:\n\
         \x20     - target/vgit/changed-files.txt\n\
         \x20     - target/vgit/vti-plan.json\n\
         \x20     - target/vgit/vti-skipped.json\n\n",
...
pub fn emit_external_gitlab_yaml(plan: &ExternalTestPlan, workspace: Option<&Path>) -> String {
    match &plan.mode {
        ...
        ExternalPlanMode::Full | ExternalPlanMode::Selected => {
            ...
            yaml.push_str("# Auto-generated by vgit Test Intelligence.\n");
...
pub fn explain_external_plan(plan: &ExternalTestPlan) -> String {
    ...
    out.push_str("╭─ vgit Test Intelligence Plan (external) ─────╮\n");
