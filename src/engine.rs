...
async fn system_health_loop(state: SharedState) {
    ...
    if let Ok(fs) = crate::cache::df_usage("/").await {
        if fs.used_percent < 85.0 {
            // Safe to let engine settle
            tokio::time::sleep(std::time::Duration::from_secs(60)).await;
        } else {
            warn!(
                "startup pre-flight check detected critical disk pressure, bypassing settle delay!"
            );
        }
    }
    ...
    loop {
        ...
        let is_critical = usage_pct > 65.0;
        let is_emergency = usage_pct > 80.0;
        let is_warning = usage_pct > 50.0;

        if !is_warning {
            debug!(usage_pct, "disk pressure nominal");
            consecutive_zero_freed = 0;

            // Always run lightweight orphan-manager GC even when healthy
        ...
        // Watermark loop: keep cleaning until usage drops below 45%.
        // INVARIANTS:
        // - Pass limit must handle worst-case 97% → 45% at ~3% freed/pass: ceil(52/3)=18.
        //   Limit is 20 to give headroom; stall detection exits early if GC has nothing left.
        // - Stall detection breaks when two consecutive passes each free < 0.5% — GC is stuck.
        //   last_freed_pct is f32::MAX so the first pass never trips the two-in-a-row check.
        // - Emergency tier (>80%) must evict active managers and enforce a hard 20 GB cap.
        // - gc_disk_cache_with_pressure must receive the current pressure flags, never the
        //   unconditional 6h/keep-active version.
        let usage_before = usage_pct;
        let mut current_usage = usage_pct;
        let mut pass = 0u32;
        let target_watermark = 45.0;
        let mut last_freed_pct = f64::MAX;

        while current_usage > target_watermark && pass < 20 {
            pass += 1;
            let escalated = pass > 1 || is_critical;
            let usage_before_pass = current_usage;

            warn!(
                usage_pct = current_usage,
                pass,
                critical = escalated,
            ...
            match crate::cache::df_usage("/").await {
                Ok(fs) => current_usage = fs.used_percent,
                Err(_) => break,
            }

            let pass_freed = usage_before_pass - current_usage;
            if pass > 2 && pass_freed < 0.5_f64 && last_freed_pct < 0.5_f64 {
                warn!(
                    pass,
                    usage_pct = current_usage,
                    "GC stalled — two consecutive passes freed < 0.5%, stopping early"
                );
                break;
            }
            last_freed_pct = pass_freed;
        ...
        let freed_pct = usage_before - current_usage;
        let _ = state
            .db
            .append_event(
        ...
        // drop(_guard) happens here normally, resetting the flag
