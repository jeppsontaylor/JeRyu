...
- VTI proof receipts for internal plans plus `vgit test select --emit-receipt`.
...
- Deterministic `vgit tui --capture` PNG export for paper, review, and agent evidence workflows.
...
- Postgres-primary state backend with SQLite fallback, bootstrap-managed Postgres Compose service, optional `VGIT_TEST_POSTGRES_URL` smoke coverage, and a disposable `just postgres-state-proof` harness.
...
- GitLab TLS certificate validation is secure by default; insecure cert acceptance requires explicit `VGIT_GITLAB_INSECURE_TLS`.
